    ...
    override func spec() {
        ...
        describe("Generator") {
            ...
            var beforeEachGenerate: () -> Void = {
                let fooType = Class(name: "Foo", variables: [Variable(name: "intValue", typeName: TypeName("Int"))], inheritedTypes: ["NSObject", "Decodable", "AlternativeProtocol"], isGeneric: true, genericTypes: [Generic(name: "T"), Generic(name: "Element")])
            ...
            describe("accessing specific type via type.Typename") {
                ...
                it("can access genericTypes") {
                    expect(generate("Foo has {{ type.Foo.genericTypes.count }} genericTypes, first: {{ type.Foo.genericTypes.first.name }}")).to(equal("Foo has 2 genericTypes, first: T"))
                }
