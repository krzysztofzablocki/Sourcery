        ...
        describe("FileParser") {
            describe("parseMethod") {
                ...
                it("extracts methods properly") {
                    let methods = parse("class Foo { init(); func bar(some: Int) throws ->Bar {}; func foo() ->  \n  Foo {}; func fooBar() rethrows {}; func fooVoid(){}; func fooInOut(some: Int, anotherSome: inout String)\n{} deinit {} }")[0].methods

                    expect(methods[0]).to(equal(Method(name: "init()", selectorName: "init", parameters: [], returnTypeName: TypeName(""), definedInTypeName: TypeName("Foo"))))
                ...
                it("extracts protocol methods properly") {
                    let methods = parse("protocol Foo { init(); func bar(some: Int) throws ->Bar ; func foo() ->    Foo ; func fooBar() rethrows ; func fooVoid(); func fooInOut(some: Int, anotherSome: inout String) }")[0].methods
                    expect(methods[0]).to(equal(Method(name: "init()", selectorName: "init", parameters: [], returnTypeName: TypeName(""), definedInTypeName: TypeName("Foo"))))
