                ...
                it("extracts subscripts properly") {
                    ...
                    expect(subscripts?[0]).to(equal(
                        Subscript(parameters: [
                            ...
                            ], returnTypeName: TypeName("Int"), accessLevel: (.private, .private), attributes: [Attribute.Identifier.final.name: Attribute(name: "final", description: "final")], annotations: [:], definedInTypeName: TypeName("Foo"))
                    ))

                    expect(subscripts?[1]).to(equal(
                        Subscript(parameters: [
                            MethodParameter(name: "b", typeName: TypeName("Int"))
                            ], returnTypeName: TypeName("String"), accessLevel: (.public, .private), attributes: [:], annotations: [:], definedInTypeName: TypeName("Foo"))
                        )
                    )
                }
