...
- `--force-parse` - File extensions of Sourcery generated file you want to parse. You can provide multiple extension using multiple `--force-parse` options. (i.e. `file.toparse.swift` will be parsed even if generated by Sourcery if `--force-parse toparse`). Useful when trying to implement a multiple phases generation. Any inline code with annotation ending with "." and this option would also be parsed(i.e. inline code beginning with // sourcery:inline:auto:Command.MirrorEnum would be parsed even if generated by Sourcery if `--force-parse MirrorEnum`).
