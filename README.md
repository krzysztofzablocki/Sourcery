...
- `--force-parse` - normally Sourcery does not process files or code within inline annotations generated by Sourcery itself. To force parsing of such files or code within inline annotations pass the extension (without `.swift`) of the file or the name of the annotation to this option. I.e. to process file `*.generated.swift` you can use `--force-parse generated`, to process code within `sourcery:inline:auto:Type.AutoCodable` annotation you can use `--force-parse AutoCodable`. You can provide this option multiple times. Useful when trying to implement a multiple phases generation.
