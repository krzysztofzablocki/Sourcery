...
final class Parser {
    ...
    public func parseFile(_ path: Path, existingTypes: [Type] = []) throws -> ([Type], [String: String]) {
    ...
    /// - Returns: All types we could find.
    /// - Throws: parsing errors.
    public func parseContents(_ contents: String, existingTypes: [Type] = []) -> (types: [Type], typealiases: [String: String]) {
        ...
        let types = parseTypes(Structure(file: file).dictionary, existingTypes: existingTypes)
        let typealiases = parseTypeAliases(SyntaxMap(file: file).tokens)
        return (types, typealiases)
    }
    
    internal func parseTypeAliases(_ tokens: [SyntaxToken]) -> [String: String] {
    ...
    /// - Returns: Just types.
    internal func uniqueTypes(_ types: [Type], typealiases: [String: String]) -> [Type] {
        ...
        for (_, type) in unique {
            for variable in type.variables {
                if let actualTypeName = typealiases[variable.__unwrappedTypeName] {
