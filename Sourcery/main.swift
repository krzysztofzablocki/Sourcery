...
func runCLI() {
    command(
        ...
        do {
            ...
            if let keepAlive = try Sourcery(verbose: verboseLogging, watcherEnabled: watcherEnabled, cacheDisabled: disableCache, arguments: args.arguments).processFiles(sources, usingTemplates: templates, output: output) {
                RunLoop.current.run()
                _ = keepAlive
            } else {
